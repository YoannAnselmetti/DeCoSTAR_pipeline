...
###         (ALL / 50pourc)
###
###   Name: run_TRIMMING.sh                    Author: Yoann Anselmetti
###   Creation date: 2015/09/21                Last modification: 2016/12/09
###
   ...
   for DirFil in $DirList; do
      if $(echo "$DirFil" | grep -q "SRR"); then
         ...
         if [[ ${TRIMMING_tool} == "ERNE-filter" ]]; then
            ...
            DIR=$(echo $1/$SRR | sed "s/FASTQ\/RAW/FASTQ\/${TRIMMING_tool}${trimmomatic_opt}/g")
         else
         ...
         if [[ ${TRIMMING_tool} == "ERNE-filter" ]]; then
            ...
            if [[ ${trimmomatic_opt} == "1" ]]; then
               ...
               jobName="trimmomatic_opt${trimmomatic_opt}_${SRR}_$TAG"

            elif [[ ${trimmomatic_opt} == "2" ]]; then
               ...
               echo "ERROR, option parameter for TRIMMOMATIC should be \"1\", \"2\" or \"3\"."
               exit 1
            fi
